...
import br.com.blinkdev.leadsponge.end_points.product.entity.ProductEntity_;
...
public class ProductRepositoryImpl implements ProductRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(ProductFilter produtoFilter, CriteriaBuilder builder, Root<ProductEntity> root) {
        ...
        if (produtoFilter.getName() != null && !produtoFilter.getName().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(ProductEntity_.name)), "%" + produtoFilter.getName().toLowerCase() + "%"));
        }
        if (produtoFilter.getDescription() != null && !produtoFilter.getDescription().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(ProductEntity_.description)), "%" + produtoFilter.getDescription().toLowerCase() + "%"));
        }
