...
public class ContatoRepositoryImpl implements ContatoRepositoryQuery {
	...
	@Override
	public Page<ContatoEntity> filtrar(ContatoFilter contatoFilter, Pageable pageable) {
		CriteriaBuilder builder = manager.getCriteriaBuilder();
		CriteriaQuery<ContatoEntity> criteria = builder.createQuery(ContatoEntity.class);
		Root<ContatoEntity> root = criteria.from(ContatoEntity.class);

		Predicate[] predicates = criarRestricoes(contatoFilter, builder, root);
		criteria.where(predicates);

		TypedQuery<ContatoEntity> query = manager.createQuery(criteria);
