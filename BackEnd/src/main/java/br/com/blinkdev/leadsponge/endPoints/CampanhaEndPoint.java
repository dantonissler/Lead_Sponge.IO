...
public class CampanhaEndPoint {
    ...
    public ResponseEntity<PagedModel<CampanhaModel>> getCampanhaByFilter(CampanhaFilter campanhaFilter, Pageable pageable) {
        return new ResponseEntity<>(campanhaService.getCampanhaByFilter(campanhaFilter, pageable), HttpStatus.OK);
    }
    ...
    @PutMapping()
    @PreAuthorize("hasAuthority('CADASTRAR_CAMPANHA') and #oauth2.hasScope('write')")
    public ResponseEntity<CampanhaEntity> atualizar(@Valid @RequestBody CampanhaEntity campanha, HttpServletResponse response) {
        CampanhaEntity novaCampanha = campanhaService.atualizar(campanha);
