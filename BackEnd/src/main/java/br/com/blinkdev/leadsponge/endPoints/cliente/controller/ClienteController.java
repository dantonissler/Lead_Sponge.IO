package br.com.blinkdev.leadsponge.endPoints.cliente.controller;
...
class ClienteController {
    ...
    Page<ClienteEntity> list(ClienteFilter clienteFilter, Pageable pageable) {
    ...
    ResponseEntity<ClienteEntity> cadastrar(@Valid @RequestBody ClienteEntity cliente, HttpServletResponse response) {
        ClienteEntity criarCliente = clienteService.salvar(cliente);
    ...
    @PutMapping(value = {"/{id}"})
    @PreAuthorize("hasAuthority('CADASTRAR_CLIENTE') and #oauth2.hasScope('write')")
    ResponseEntity<ClienteEntity> atualizar(@Valid @RequestBody ClienteEntity cliente, @PathVariable Long id, HttpServletResponse response) {
        ClienteEntity novoCliente = clienteService.atualizar(id, cliente);
    ...
    @GetMapping(value = {"/{id}"})
    @PreAuthorize("hasAuthority('PESQUISAR_CLIENTE') and #oauth2.hasScope('read')")
    ResponseEntity<ClienteEntity> detalhar(@Valid @PathVariable("id") Long id, HttpServletResponse response) {
