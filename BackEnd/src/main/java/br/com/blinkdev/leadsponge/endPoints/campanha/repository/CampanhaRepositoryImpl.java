...
import br.com.blinkdev.leadsponge.endPoints.campanha.entity.CampanhaEntity_;
...
public class CampanhaRepositoryImpl implements CampanhaRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(CampanhaFilter campanhaFilter, CriteriaBuilder builder, Root<CampanhaEntity> root) {
        ...
        if (campanhaFilter.getNome().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(CampanhaEntity_.nome)), "%" + campanhaFilter.getNome().toLowerCase() + "%"));
        }
        if (campanhaFilter.getDescricao().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(CampanhaEntity_.descricao)), "%" + campanhaFilter.getDescricao().toLowerCase() + "%"));
        }
