...
import br.com.blinkdev.leadsponge.endPoints.negotiationStyle.entity.NegotiationStyleEntity_;
...
public class NegotiationStyleRepositoryImpl implements NegotiationStyleRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(NegotiationStyleFilter estagioNegociacaoFilter, CriteriaBuilder builder, Root<NegotiationStyleEntity> root) {
        ...
        if (estagioNegociacaoFilter.getNome() != null && !estagioNegociacaoFilter.getNome().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(NegotiationStyleEntity_.nome)), "%" + estagioNegociacaoFilter.getNome().toLowerCase() + "%"));
        }
