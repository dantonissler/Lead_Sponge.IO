...
import br.com.blinkdev.leadsponge.end_points.segment.entity.SegmentEntity_;
...
public class SegmentRepositoryImpl implements SegmentRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(SegmentFilter segmentoFilter, CriteriaBuilder builder, Root<SegmentEntity> root) {
        ...
        if (segmentoFilter.getName() != null && !segmentoFilter.getName().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(SegmentEntity_.name)), "%" + segmentoFilter.getName().toLowerCase() + "%"));
        }
