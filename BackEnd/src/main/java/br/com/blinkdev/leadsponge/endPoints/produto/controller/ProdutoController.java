package br.com.blinkdev.leadsponge.endPoints.produto.controller;
...
class ProdutoController extends ErroMessage {
    ...
    public Page<ProdutoEntity> list(ProdutoFilter produtoFilter, Pageable pageable) {
    ...
    public ResponseEntity<ProdutoEntity> cadastrar(@Valid @RequestBody ProdutoEntity produto, HttpServletResponse response) {
        ProdutoEntity criarProduto = produtoService.salvar(produto);
    ...
    @PutMapping(value = {"/{id}"})
    @PreAuthorize("hasAuthority('CADASTRAR_PRODUTO') and #oauth2.hasScope('write')")
    ResponseEntity<ProdutoEntity> atualizar(@Valid @RequestBody ProdutoEntity produto, @PathVariable Long id, HttpServletResponse response) {
        ProdutoEntity novaProduto = produtoService.atualizar(id, produto);
    ...
    public ResponseEntity<ProdutoEntity> deletar(@PathVariable Long id) {
        return ResponseEntity.ok(produtoService.deletar(id));
    }

    @GetMapping(value = {"/{id}"})
    @PreAuthorize("hasAuthority('PESQUISAR_PRODUTO') and #oauth2.hasScope('read')")
    public ResponseEntity<ProdutoEntity> detalhar(@Valid @PathVariable("id") Long id, HttpServletResponse response) {
    ...
    public ResponseEntity<ProdutoEntity> atualizarPropriedadeVisibilidade(@PathVariable Long id, @RequestBody Boolean visibilidade) {
