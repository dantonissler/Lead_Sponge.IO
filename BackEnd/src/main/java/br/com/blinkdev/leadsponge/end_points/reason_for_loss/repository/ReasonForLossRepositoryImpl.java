...
import br.com.blinkdev.leadsponge.end_points.reason_for_loss.entity.ReasonForLossEntity_;
...
public class ReasonForLossRepositoryImpl implements ReasonForLossRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(ReasonForLossFilter motivoPerdaFilter, CriteriaBuilder builder, Root<ReasonForLossEntity> root) {
        ...
        if (motivoPerdaFilter.getName() != null && !motivoPerdaFilter.getName().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(ReasonForLossEntity_.name)), "%" + motivoPerdaFilter.getName().toLowerCase() + "%"));
        }
