...
import br.com.blinkdev.leadsponge.end_points.contact.entity.ContactEntity_;
...
public class ContactRepositoryImpl implements ContactRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(ContactFilter contatoFilter, CriteriaBuilder builder, Root<ContactEntity> root) {
        ...
        if (contatoFilter.getName() != null && !contatoFilter.getName().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(ContactEntity_.name)), "%" + contatoFilter.getName().toLowerCase() + "%"));
        }
        if (contatoFilter.getOffice() != null && !contatoFilter.getOffice().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(ContactEntity_.office)), "%" + contatoFilter.getOffice().toLowerCase() + "%"));
        }
