...
import br.com.blinkdev.leadsponge.endpoints.negotiationSource.entity.NegotiationSourceEntity_;
...
public class NegotiationSourceRepositoryImpl implements NegotiationSourceRepositoryQuery {
    ...
    private Predicate[] criarRestricoes(NegotiationSourceFilter fonteNegociacaoFilter, CriteriaBuilder builder, Root<NegotiationSourceEntity> root) {
        ...
        if (fonteNegociacaoFilter.getNome() != null && !fonteNegociacaoFilter.getNome().isBlank()) {
            predicates.add(builder.like(builder.lower(root.get(NegotiationSourceEntity_.nome)), "%" + fonteNegociacaoFilter.getNome().toLowerCase() + "%"));
        }
